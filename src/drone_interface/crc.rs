// https://github.com/Alexander89/rust-tello/blob/master/src/crc.rs#L52
...
/// calculates the starting CRC8 byte for packet.
pub fn crc8_ref<'a>(pkt: impl IntoIterator<Item = &'a u8>) -> u8 {
	pkt.into_iter()
		.fold(0x77u8, |crc, x| CRC8_TABLE[(crc ^ x) as usize])
}
...
pub fn crc16_ref<'a>(pkt: impl IntoIterator<Item = &'a u8>) -> u16 {
	pkt.into_iter().fold(0x3692u16, |crc, x| {
		CRC16_TABLE[((crc ^ *x as u16) & 0xff) as usize] ^ (crc >> 8)
	})
}